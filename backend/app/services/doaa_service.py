...
doaa_procedures = [
    ...
    },
    {
        ...
        "steps": "1. Institute Merit Scholarships (Merit-I, Merit-II, Merit-III) are awarded based on AGPA/CGPA announced after EST results.\n2. Means-cum-Merit scholarships: Submit income certificate (below income threshold) to DoAA / Student Affairs Office during notice window.\n3. Government scholarships (NSP, State portals): Submit verification documents to Academic Section.",
    },
]

OFFICIAL_DOAA_LINK = "https://docs.google.com/document/d/18kHw-1pyXyKauhNwv6QlePw2JVbgUszVbgQN5fa2Ons/edit?tab=t.0"

def get_doaa_procedure_by_id(proc_id: Any) -> Optional[str]:
    procedure = next((p for p in doaa_procedures if str(p["id"]) == str(proc_id)), None)
    if procedure:
        return f"**Regarding \"{procedure['task']}\":**\n\n{procedure['steps']}\n\n**For official DoAA guidelines and circulars:**\n[Official DoAA Document]({OFFICIAL_DOAA_LINK})"
    return None

def find_doaa_procedure(prompt: str) -> Optional[str]:
    if not prompt:
        return None

    lower_prompt = prompt.lower()
    ignore_words = {"the", "for", "and", "how", "what", "can", "tell", "about", "get"}
    prompt_tokens = {w for w in re.split(r"\s+", lower_prompt) if len(w) > 2 and w not in ignore_words}

    best_match = None
    max_score = 0

    for proc in doaa_procedures:
        current_score = 0
        ...
        # Task token overlap
        task_tokens = {w for w in re.split(r"\s+", proc["task"].lower()) if len(w) > 2}
        intersection = prompt_tokens.intersection(task_tokens)
